...
#endif
#if __ANDROID__
using NativeUIElement = Android.Views.View;
#elif __IOS__
using NativeUIElement = UIKit.UIView;
#endif
...
namespace Nventive.View.Controls
{
	public partial class StarStackPanel : Panel
	{
...
#if !WINDOWS_UWP && !__ANDROID__ && !__IOS__ && !__WINDOWS10_0_18362_0__ // Useless operator (==overhead on UI thread) for Jupiter platform
				.Cast<UIElement>()
#endif
				.OrderBy(GetPriority)
				.ToArray();
		...
		private void MesureChildAbsolute(UIElement child, Orientation orientation, Size availableSize, GridLength sizeHint, ref Size totalSize)
		{
			...
			var desiredSize = MeasureElement((NativeUIElement)child, availableSize);
#else
		...
		protected override Size ArrangeOverride(Size finalSize)
		{
			...
			var children = Children
#if !WINDOWS_UWP // Useless operator (==overhead on UI thread) for Jupiter platform
				.Cast<UIElement>()
#endif
		...
		{
#if __ANDROID__ || __IOS__
			var desiredSize = GetElementDesiredSize((NativeUIElement)child);
#else
			var desiredSize = child.DesiredSize;
#endif
