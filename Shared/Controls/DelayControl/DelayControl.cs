#if WINDOWS_UWP || __ANDROID__ || __IOS__ || __MACOS__ || __WASM__ || WINDOWS
using System;
...
namespace Nventive.View.Controls
{
	...
	public partial class DelayControl : Control
	{
		...
		/// </summary>
#if WINUI
		public DispatcherQueuePriority ApplyTemplatePriority
#else
...
#endif
				)GetValue(ApplyTemplatePriorityProperty);
			}
			set { SetValue(ApplyTemplatePriorityProperty, value); }
		}

		public static readonly DependencyProperty ApplyTemplatePriorityProperty =
			DependencyProperty.Register("ApplyTemplatePriority", typeof(
#if WINUI
				 DispatcherQueuePriority
#else
				 CoreDispatcherPriority 
#endif
				), typeof(DelayControl), new PropertyMetadata(
#if WINUI
		...
		/// </summary>
#if WINUI
		public DispatcherQueuePriority InnerViewPriority
#else
		...
		public DelayControl()
		{
			...
			_dispatcher = new MainDispatcherScheduler(
#if WINUI
						DispatcherQueue
#else
						Dispatcher
		...
		private void ScheduleMaterialize(
			TimeSpan delay,
#if WINUI
		 DispatcherQueuePriority applyTemplatePriority,
		 DispatcherQueuePriority innerViewPriority
#else
		 CoreDispatcherPriority applyTemplatePriority,
		 CoreDispatcherPriority innerViewPriority 
#endif
			)
		{
