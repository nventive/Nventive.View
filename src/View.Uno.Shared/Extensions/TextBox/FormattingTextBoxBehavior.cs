...
namespace Nventive.View.Extensions
{
	...
	public partial class FormattingTextBoxBehavior
	{
		...
		#region DependencyProperty: PreviousTextLength

		private static DependencyProperty PreviousTextLengthProperty { get; } = DependencyProperty.RegisterAttached(
			"PreviousTextLength",
			typeof(int),
			typeof(FormattingTextBoxBehavior),
			new PropertyMetadata(default));

		private static int GetPreviousTextLength(TextBox obj) => (int)obj.GetValue(PreviousTextLengthProperty);
		private static void SetPreviousTextLength(TextBox obj, int value) => obj.SetValue(PreviousTextLengthProperty, value);
		...
		public static readonly DependencyProperty RawTextProperty =
			DependencyProperty.RegisterAttached("RawText", typeof(string), typeof(FormattingTextBoxBehavior), new PropertyMetadata(string.Empty));
...
#pragma warning restore Uno0001 // Uno type or member is not implemented
		...
		private static void OnTextChanged(object sender, TextChangedEventArgs e)
		{
			...
			if (GetPreviousTextLength(textbox) > length)
			{
				SetPreviousTextLength(textbox, length);
				return;
			}
		...
		private static void FormatText(TextBox textbox, string textFormat)
		{
			...
			var dispatcherScheduler = new MainDispatcherScheduler(
#if WINUI
						textbox.DispatcherQueue
#else
						textbox.Dispatcher
#endif
						);

			dispatcherScheduler.Schedule(() =>
			{
				textbox.Text = formattedText;
				textbox.SelectionStart = selectionStart;
				SetPreviousTextLength(textbox, formattedText.Length);
			});
		}
		...
		private static string FormatInput(string text, string format, int maskedIndex, int textIndex, ref int cursorIndex)
		{
			...
			while (maskedIndex < format.Length && textIndex < text.Length)
			{
				...
				{
					var nextAlphaNumericIndex = GetNextAlphanumericIndex(text, textIndex, sb.Length, ref cursorIndex);
				...
				{
					var nextDigit = GetNextDigitIndex(text, textIndex, sb.Length, ref cursorIndex);
					if (nextDigit != -1)
					{
						textIndex = nextDigit;
						sb.Append(text[textIndex]);
				...
				{
					var nextLetterIndex = GetNextCharacterIndex(text, textIndex, sb.Length, ref cursorIndex);
		...
		private static int GetNextDigitIndex(string text, int currentIndex, int bufferLength, ref int cursorIndex)
		{
		...
		private static int GetNextCharacterIndex(string text, int currentIndex, int bufferLength, ref int cursorIndex)
		{
			while (currentIndex < text.Length)
			{
		...
		private static int GetNextAlphanumericIndex(string text, int currentIndex, int bufferLength, ref int cursorIndex)
		{
			while (currentIndex < text.Length)
			{
